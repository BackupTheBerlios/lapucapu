 ...
 *
 * $Id: ReadExample.cpp,v 1.1 2006/05/24 18:44:03 xvier Exp $
 */
...
void AccessExample::run(bool removeExistingDatabase, const char *fileName)
{
	...
	db.set_errpfx("ReadExample");
	db.set_pagesize(1024);		/* Page size: 1K. */
	db.set_cachesize(0, 32 * 1024, 0);
	db.open(NULL, fileName, NULL, DB_BTREE, 0, 0664);
	...
	for (;;) {
		...
		*t++ = '\0';
		...
		ret = db.get(0, &key, &data, 0);
		//ret = db.put(0, &key, &data, DB_NOOVERWRITE);
		if (ret == DB_NOTFOUND) {
			cout << "Key " << buf << " not found!\n";
			string str1;
			str1.append((char*)data.get_data(), len);
			cout << "Data: " << str1 << endl;
		}
		else {
			string str1;
